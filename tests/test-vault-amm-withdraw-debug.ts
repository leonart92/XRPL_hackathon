...
console.log("🧪 Testing Vault AMM Withdrawal - Debug\n");
...
let balance = await xrplService.getAccountBalance(userWallet.address);
console.log("\n📊 After Deposit:");
console.log("   User TST:", balance.tokens.find((t) => t.currency === "TST")?.balance || "0");
console.log("   User VTS:", balance.tokens.find((t) => t.currency === "VTS")?.balance || "0");

let vaultBalance = await xrplService.getAccountBalance(vaultWallet.address);
console.log("   Vault TST:", vaultBalance.tokens.find((t) => t.currency === "TST")?.balance || "0");
...
console.log("⏳ Waiting for withdrawal processing...");
await new Promise((r) => setTimeout(r, 10000));

balance = await xrplService.getAccountBalance(userWallet.address);
vaultBalance = await xrplService.getAccountBalance(vaultWallet.address);
